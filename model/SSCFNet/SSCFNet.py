...
import copy
import math
from torch.nn import Dropout, Softmax, Conv2d
...
from model.SSCFNet.plugin_module import SPM
...
class Attention_org(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.psi = nn.InstanceNorm2d(self.num_attention_heads)# 样本归一化
...
# einops rearrange，(b,c,h,w) to (b,(h*w),c) 
def to_3d(x):
    return rearrange(x, 'b c h w -> b (h w) c')

# einops rearrange，(b,(h*w),c)  to (b,c,h,w) 
def to_4d(x, h, w):
...
# (x - mean(x)) / (sqrt(var(x))) * weight + bias
class WithBias_LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        sigma = x.var(-1, keepdim=True, unbiased=False) 
        tmp1=x-mu
        tmp2=torch.sqrt(sigma + 1e-5)
        tmp3=tmp1 / tmp2
        tmp4=tmp3 * self.weight 
        tmp5=tmp4 + self.bias 
...
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        hidden_features = int(dim * ffn_expansion_factor) # c_hidden

        self.project_in = nn.Conv2d(dim, hidden_features * 2, kernel_size=1, bias=bias) # 1*1 convolution
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.attn_norm1 = LayerNorm2d(channel_num[0], LayerNorm_type='WithBias')
        self.attn_norm2 = LayerNorm2d(channel_num[1], LayerNorm_type='WithBias')
        self.attn_norm3 = LayerNorm2d(channel_num[2], LayerNorm_type='WithBias')
        self.attn_norm4 = LayerNorm2d(channel_num[3], LayerNorm_type='WithBias')
        self.attn_norm = LayerNorm2d(config.KV_size, LayerNorm_type='WithBias')

        self.channel_attn = Attention_org(config, vis, channel_num)

        self.ffn_norm1 = LayerNorm2d(channel_num[0], LayerNorm_type='WithBias')
        self.ffn_norm2 = LayerNorm2d(channel_num[1], LayerNorm_type='WithBias')
        self.ffn_norm3 = LayerNorm2d(channel_num[2], LayerNorm_type='WithBias')
        self.ffn_norm4 = LayerNorm2d(channel_num[3], LayerNorm_type='WithBias')
...
class Encoder(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.encoder_norm1 = LayerNorm2d(channel_num[0], LayerNorm_type='WithBias')
        self.encoder_norm2 = LayerNorm2d(channel_num[1], LayerNorm_type='WithBias')
        self.encoder_norm3 = LayerNorm2d(channel_num[2], LayerNorm_type='WithBias')
        self.encoder_norm4 = LayerNorm2d(channel_num[3], LayerNorm_type='WithBias')
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_feature):
        super(MLP, self).__init__()
        self.linear1 = nn.Linear(input_dim, hidden_feature)
        self.linear2 = nn.Linear(hidden_feature, input_dim)
...
class SSCFNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        block = Res_block # resnet block
        self.pool = nn.MaxPool2d(2, 2)
        self.inc = self._make_layer(block, n_channels, in_channels)# default 1
        ...
        self.learnable_shape_prior = nn.Parameter(torch.randn(1, 256))
        self.spm0 = SPM(config, 256, (2, 2))
        self.spm1 = SPM(config, 256, (4, 4))
        self.spm2 = SPM(config, 128, (8, 8))
        self.spm3 = SPM(config, 64, (16, 16))
        self.spm4 = SPM(config, 32, (32, 32))
        tmp=in_channels*8
        self.con_after_cat0 = nn.Conv2d(in_channels * 16, in_channels * 8, 1)
        self.con_after_cat1 = nn.Conv2d(in_channels * 16, in_channels * 8,1)
        self.con_after_cat2 = nn.Conv2d(in_channels * 8, in_channels * 4, 1)
        self.con_after_cat3 = nn.Conv2d(in_channels * 4, in_channels * 2, 1)
        self.con_after_cat4 = nn.Conv2d(in_channels * 2, in_channels * 1, 1)

        self.act_after_cat0 = nn.Sequential(
            Conv2dReLU(in_channels * 16, in_channels * 8, kernel_size=3, padding=1),
            Conv2dReLU(in_channels * 8, in_channels * 8, kernel_size=3, padding=1)
        )
        self.act_after_cat1 = nn.Sequential(
            Conv2dReLU(in_channels * 16, in_channels * 8, kernel_size=3, padding=1),
            Conv2dReLU(in_channels * 8, in_channels * 8, kernel_size=3, padding=1)
        )
        self.act_after_cat2 = nn.Sequential(
            Conv2dReLU(in_channels * 8, in_channels * 4, kernel_size=3, padding=1),
            Conv2dReLU(in_channels * 4, in_channels * 4, kernel_size=3, padding=1)
        )
        self.act_after_cat3 = nn.Sequential(
            Conv2dReLU(in_channels * 4, in_channels * 2, kernel_size=3, padding=1),
            Conv2dReLU(in_channels * 2, in_channels * 2, kernel_size=3, padding=1)
        )
        self.act_after_cat4 = nn.Sequential(
            Conv2dReLU(in_channels * 2, in_channels * 1, kernel_size=3, padding=1),
            Conv2dReLU(in_channels * 1, in_channels * 1, kernel_size=3, padding=1)
        )

        self.relu = nn.LeakyReLU(inplace=True)

        self.con_after_add0 = nn.Conv2d(in_channels * 8, in_channels * 8, 3,padding=1)
        self.con_after_add1 = nn.Conv2d(in_channels * 8, in_channels * 8, 3,padding=1)
        self.con_after_add2 = nn.Conv2d(in_channels * 4, in_channels * 4, 3,padding=1)
        self.con_after_add3 = nn.Conv2d(in_channels * 2, in_channels * 2, 3,padding=1)
        self.con_after_add4 = nn.Conv2d(in_channels * 1, in_channels * 1, 3,padding=1)
    ...
    def forward(self, x):
        ...
        learnable_shape_prior = self.learnable_shape_prior.repeat(batch_size, 1, 1)

        x1 = self.inc(x)  

        class_feature4, refined_shape_prior = self.spm4(x1,learnable_shape_prior)  
  
        x_add = x1 + class_feature4  # (4,32,512,512)
        x_add_norm = self.relu(x_add)
        x_conv_after_add_norm = self.con_after_add4(x_add_norm)
        x1 = x_conv_after_add_norm

        x2 = self.down_encoder1(self.pool(x1))  

        class_feature3, refined_shape_prior = self.spm3(x2,refined_shape_prior)  # feature (2,256,32,32) shape(2,1,256)
        x_add = x2 + class_feature3  
        x_add_norm = self.relu(x_add)
        x_conv_after_add_norm = self.con_after_add3(x_add_norm)
        x2 = x_conv_after_add_norm
        x3 = self.down_encoder2(self.pool(x2))  

        class_feature2, refined_shape_prior = self.spm2(x3,refined_shape_prior)  
 
        x_add = x3 + class_feature2 
        x_add_norm = self.relu(x_add)
        x_conv_after_add_norm = self.con_after_add2(x_add_norm)
        x3 = x_conv_after_add_norm
 
        x4 = self.down_encoder3(self.pool(x3))

        class_feature1, refined_shape_prior = self.spm1(x4,refined_shape_prior)  

        x_add = x4 + class_feature1  
        x_add_norm = self.relu(x_add)
        x_conv_after_add_norm = self.con_after_add1(x_add_norm)
        x4 = x_conv_after_add_norm

        d5 = self.down_encoder4(self.pool(x4))  

        class_feature0, refined_shape_prior = self.spm0(d5, refined_shape_prior) 
        x_add = d5 + class_feature0
        x_add_norm = self.relu(x_add)
        x_conv_after_add_norm = self.con_after_add0(x_add_norm)
        d5 = x_conv_after_add_norm

        f1 = x1
        f2 = x2
        f3 = x3
        f4 = x4
        x1, x2, x3, x4, att_weights = self.mtc(x1, x2, x3, x4) # channel attention
        ...
        out = self.outc(self.up_decoder1(d2, x1)) # (1,1,256,256)

        # deep supervision
        if self.deepsuper: # defualt true
...
def test_func():
    ...
    model = SSCFNet(config_vit, mode='train', deepsuper=True)
